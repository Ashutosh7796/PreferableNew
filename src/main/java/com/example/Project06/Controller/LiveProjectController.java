...
public class LiveProjectController {
    ...
    @PatchMapping("/update/{liveProjectID}")
    public ResponseEntity<ResponseDto> updateLiveProject(@RequestBody LiveProjectDto liveProjectDto, @PathVariable Integer liveProjectID) {
        try {
            String result = liveProjectService.EditLiveProject(liveProjectDto, liveProjectID);
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseDto("success", result));
        } catch (LiveProjectNotFoundException liveProjectNotFoundException) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseDto("unsucces", "Live Project not found"));
        }
    }
