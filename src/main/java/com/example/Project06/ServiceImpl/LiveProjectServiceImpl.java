...
public class LiveProjectServiceImpl implements LiveProjectService {
    ...
    public String EditLiveProject(LiveProjectDto liveProjectDto, Integer liveProjectID) {
        ...
        LiveProject liveProject = liveProjectRepo.findById(liveProjectID).orElseThrow(() -> new LiveProjectNotFoundException("Not Found", HttpStatus.NOT_FOUND));

        if (liveProjectDto.getCompany() != null) {
            liveProject.setCompany(liveProjectDto.getCompany());
        }

        if (liveProjectDto.getDomain() != null) {
            liveProject.setDomain(liveProjectDto.getDomain());
        }

        if (liveProjectDto.getInternshipType() != null) {
            liveProject.setInternshipType(liveProjectDto.getInternshipType());
        }

        if (liveProjectDto.getInternshipDuration() != null) {
            liveProject.setInternshipDuration(liveProjectDto.getInternshipDuration());
        }

        if (liveProjectDto.getPointOfContact() != null) {
            liveProject.setPointOfContact(liveProjectDto.getPointOfContact());
        }

        if (liveProjectDto.getPointOfContact01() != null) {
            liveProject.setPointOfContact01(liveProjectDto.getPointOfContact01());
        }

        if (liveProjectDto.getLocation() != null) {
            liveProject.setLocation(liveProjectDto.getLocation());
        }

        if (liveProjectDto.getJobDescription() != null) {
            liveProject.setJobDescription(liveProjectDto.getJobDescription());
        }

        if (liveProjectDto.getKeySkill() != null) {
            liveProject.setKeySkill(liveProjectDto.getKeySkill());
        }
        liveProjectRepo.save(liveProject);

        return "Live Project Updated" + liveProjectID;
